...
1. The reducer function that takes two args, the current value and the arg
   passed to the dispatch function, and returns the next value.
2. The initial state.
3. An initializer function - a function that takes the initial value argument
   and uses the return value for the initial state. Useful for when we want to
   do lazy initialization and compute only on first render.

And returns an array of:
1. current state.
2. dispatch function. Can behave just like a "setter".
