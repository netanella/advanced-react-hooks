...
When creating a new context, instead of accessing React.useContext directly, 
we should create a custom hook that checks that we can get the context value,
and if not throw a useful error (must be used within a provider).
