...
Exposes imperative methods to developers who pass a ref prop to our 
component. It allows assigning functions and properties directly to ref.current.
It's better to make our APIs declarative if possible, so this would be used only
when there is no other way.

`React.useImperativeHandle(ref, () => { return { focusInput: () => inputRef.current.focus(), } })`
