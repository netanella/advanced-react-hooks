...
Memoization is useful for keeping referential integrity but should be used with
caution.

Instead of using useEffect in the api and expect the user to pass memoized
dependencies - create a memoized callback and let the user handle the useEffect
dependencies.

Prevent React state update on unmounted component (Warning gone in React 18 but
still good practice) by keeping a "mounted" ref that gets updated with
useEffect, and wrap the state setter function so no state will be changed when
"mounted" is false.
